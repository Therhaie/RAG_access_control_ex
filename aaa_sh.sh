...
NEED_RECREATE_VECTOR_STORE="True"
NB_USERS=20
LARGE_VALUE=1
...
for ((i=0; i<MAX_TRIES; i++)); do
    if curl -s "$SERVER_URL/health" > /dev/null; then
        ...
        echo "Waiting for embedding server to be ready... (Attempt $((i+1))/$MAX_TRIES)"
        sleep $INTERVAL
    fi

    if [[ $i -eq $((MAX_TRIES-1)) ]]; then
        echo "Error: Embedding server did not become ready after $MAX_TRIES attempts."
        exit 1
    fi
done
...
do
    ...
    start=$(date +%s.%N)
    python opti_experiment_raw_retrieve_copy.py --top-k $val --workers 64 --large-value-chunk $LARGE_VALUE --large-value-query $LARGE_VALUE >> "$NAME_LOG_FILE" 2>&1
    end=$(date +%s.%N)
    duration=$(echo "$end - $start" | bc)
