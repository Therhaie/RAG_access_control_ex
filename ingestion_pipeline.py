...
def create_vector_store(chunks, persist_directory=CHROMA_PATH):
    ...
    cid = [f'{c.metadata["triplet_index"]}_{c.metadata["document_id"]}_{c.metadata["phrase_seq"]}' for c in chunks]
...
def main(create_vector_store_flag=False):
    ...
    chunks  = load_file(path_experimental_data)
    vectore_store = create_vector_store(chunks)
    # print("\n ingestion completed")
...
# # ── Config (customize via config.py or similar module if needed)
# load_dotenv()
...
# # Output file name is preserved
# INPUT_DOC_PATH = os.path.join(os.getcwd(), 'RAGBench_whole', 'merged_dataset.json')
# OUTPUT_DOC_PATH = os.path.join(os.getcwd(), 'RAGBench_whole', 'merged_id_triplets_with_metadata2.json')
...
#     """
#     Drop-in replacement for OpenAIEmbeddings.
#     Handles rotation, extra dims, normalization.
#     """
#     def __init__(
#         self,
#         base_model: OpenAIEmbeddings = None,
#         rotate: bool = False,
#         rotation_seed: int = 42,
#         extra_dims: int = 0,
#         extra_mode: str = "zeros",
#         normalize: bool = False,
#     ):
#         self._base = base_model or OpenAIEmbeddings(
#             model=EMBED_MODEL,
...
#     def embed_documents(self, texts: List[str]) -> List[List[float]]:
#         # Rotation/augmentation logic can be added here if needed
#         raw = self._base.embed_documents(texts)
#         return raw
...
# def get_embedding_model():
#     # Adjust arguments for CustomEmbeddings if you want rotation etc.
#     return CustomEmbeddings()
...
#         question  = triplet.get("question", "")
#         response  = triplet.get("response", "")
#         documents = triplet.get("sentences", [[]])
#         id_triplet = triplet.get("id_triplets", "")
...
#                     metadata={
#                         "document_id"  : doc_id,
#                         "phrase_seq"   : phrase_seq,
#                         "triplet_index": id_triplet,
#                     }
#                 ))
#     print(f"  → {len(documents)} chunks loaded  |  {skipped} entries skipped")
#     return documents

# # --- Efficient Batch Embedding + DB Storage ---

# def batch_chunks(chunks, batch_size):
#     for i in range(0, len(chunks), batch_size):
#         yield chunks[i:i + batch_size]

# def embed_and_store_batch(docs_batch, embedding_model, vectorstore):
#     texts = [doc.page_content for doc in docs_batch]
#     metadatas = [doc.metadata for doc in docs_batch]
#     embeddings = embedding_model.embed_documents(texts)
#     vectorstore._collection.upsert(
#         ids=[f"{md['document_id']}_{md['phrase_seq']}" for md in metadatas],
#         embeddings=embeddings,
#         documents=texts,
#         metadatas=metadatas
#     )

# def create_vector_store_parallel(
#         chunks,
#         embedding_model,
#         persist_directory=CHROMA_PATH,
#         batch_size=BATCH_SIZE,
#         max_workers=MAX_WORKERS):
#     print("\n[Optimized] Creating embeddings & storing in ChromaDB …")
#     vectorstore = Chroma(
#         collection_name=COLLECTION_EXPERIMENTAL,
#         persist_directory=persist_directory,
...
#     batches = list(batch_chunks(chunks, batch_size))
#     with ThreadPoolExecutor(max_workers=max_workers) as executor:
#         futures = [
#             executor.submit(embed_and_store_batch, batch, embedding_model, vectorstore)
#             for batch in batches
#         ]
#         for future in as_completed(futures):
#             future.result()

#     print(f"[Optimized]  → Vector store saved to: {persist_directory}")
#     print(f"[Optimized]  → Total vectors: {vectorstore._collection.count()}")
#     return vectorstore

# def create_vector_store(chunks, persist_directory=CHROMA_PATH):
#     embedding_model = get_embedding_model()
#     return create_vector_store_parallel(
#         chunks, embedding_model, persist_directory=persist_directory
#     )

# def ingest_function():
#     chunks  = load_file(INPUT_DOC_PATH)
#     create_vector_store(chunks)
#     print("\nIngestion completed.")

# # Optional: Add CLI if you still want it
# if __name__ == "__main__":
#     ingest_function()